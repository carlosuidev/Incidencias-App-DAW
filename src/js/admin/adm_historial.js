    ...
    if (xhrHistorial.readyState == 4 && xhrHistorial.status == 200) {
        ...
        if (repuestaJson[0].id !== "nada") {
            repuestaJson.forEach(fila => {
                ...
                const tdProfesor = tr.insertCell(2);
                tdProfesor.innerHTML = `${fila.nombre} ${fila.apellidos}`;

                const tdEstado = tr.insertCell(3);
                ...
                const tdFecha = tr.insertCell(4);
                tdFecha.textContent = fila.fecha;

                const tdLeerMas = tr.insertCell(5);
                ...
                const tds = [tdTipo, tdProfesor, tdFecha];
                tds.forEach(td => {
...
function ficha() {
    if (xhrFicha.readyState == 4 && xhrFicha.status == 200) {
        ...
        repuestaJson.forEach(incidencia => {
            ...
            fecha.innerHTML = `<span class='font-bold mr-1'>${incidencia.nombre} ${incidencia.apellidos}</span>(${incidencia.fecha})`;
